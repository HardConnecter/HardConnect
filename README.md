...
*Windows 用户可以使用 GUI 版 [HardConnect for Windows](https://github.com/mythologyli/zju-connect-for-Windows)。*
...
对于 Ubuntu/Debian、RHEL 系、Arch 等基于 Systemd 的 Linux 发行版，除按照上述方法运行外，亦可通过以下步骤将 HardConnect 安装为系统服务，实现自动重连功能：
...
5. 在代理插件中添加对应本机节点和分流规则
   ...
   2. 请确保使用的 RVPN 服务器与 OpenWrt 直连。若未将 `stuvpn.fudan.edu.cn` 配置为直连，此域名可能匹配分流规则与其他 `zju.edu.cn` 流量一样被发往 zju-connect 代理，这会造成网络异常。
