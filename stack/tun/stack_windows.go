...
const interfaceName = "HardConnect"
...
func NewStack(easyConnectClient *client.EasyConnectClient, dnsHijack bool) (*Stack, error) {
	...
	if dnsHijack {
		command = exec.Command("netsh", "interface", "ipv4", "add", "dnsservers", "HardConnect", s.endpoint.ip.String())
	} else {
		command = exec.Command("netsh", "interface", "ipv4", "delete", "dnsservers", "HardConnect", "all")
	}
	err = command.Run()
	if err != nil {
		log.Printf("Run %s failed: %v", command.String(), err)
	...
	hook_func.RegisterTerminalFunc("Close Tun Device", func(ctx context.Context) error {
		dev.Close()
		closeCommand := exec.Command("netsh", "interface", "ipv4", "delete", "dnsservers", "HardConnect", "all")
